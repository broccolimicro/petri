...
struct simulator {
	...
	std::vector<enabled_transition> ready;
	...
	int enabled(bool allow_incomplete = false, bool sorted = false) {
		if (base == nullptr) {
			internal("", "nullptr pointer to simulator::base", __FILE__, __LINE__);
			return 0;
		}
		...
		for (vector<arc>::const_iterator a = base->arcs[place::type].begin(); a != base->arcs[place::type].end(); a++) {
			...
			auto d = lower_bound(disabled.begin(), disabled.end(), enabled_transition(a->to.index));
			if (d != disabled.end() and *d == a->to.index) {
				...
				auto e = lower_bound(result.begin(), result.end(), enabled_transition(a->to.index));
	...
	int renabled(bool sorted = false) {
		...
		for (vector<arc>::const_iterator a = base->arcs[transition::type].begin(); a != base->arcs[transition::type].end(); a++) {
			...
			bool d_invalid = (d == disabled.end() or *d != a->from.index);
			...
			if (d_invalid) {
				...
				bool found = false;
				for (int j = 0; j < (int)tokens.size() and not found; j++) {
					if (a->to.index == tokens[j].index and
						(e_invalid or find(e->tokens.begin(), e->tokens.end(), j) == e->tokens.end())) {
						// We are safe to add this to the list of possibly enabled transitions
						found = true;
						if (e_invalid) {
