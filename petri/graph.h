...
struct graph
{
	...
	virtual segment compose(int composition, segment s0, segment s1, bool proper=false) {
		mark_modified();
	...
	virtual bound select(int composition, vector<petri::iterator> nodes, bool always=false, bool invert=false) {
	...
	bound group(int composition, bound nodes, bool always=false, bool invert=false) {
	...
	bound complete(int composition, bound nodes) {
	...
	virtual void flatten() {
						...
						if (parent_sequence.empty()) {
							...
							if (parent_sequence_head == in_transition) {
								...
								for (const petri::iterator &child_transition : this->next(place_to_merge)) {
									...
									//TODO: RETVRN to splice out in+out arcs to parent_sequence_head
		...
		//TODO: prune / tree-shake newly dead nodes (up-to dominance frontier for un-marked places w. no in-transitions
